...
export const ListeCollectionScreen = ({ navigation, route }) => {
    ...
    const [temp, settemp] = React.useState(item && item['list']);
    ...
    const renderItem = ({ item }) => {
        ...
        const { commentaire, devise, prix_max, prix_min, __tbl_user, __tbl_unite_mesure, __tbl_culture } = item;
        const { cultures, ref } = __tbl_culture;
                ...
                <View style={{ borderColor: Colors.primaryColor, borderWidth: .4, padding: 10, borderRadius: 10, elevation: 2, backgroundColor: Colors.whiteColor }}>
                    ...
                    <View style={{ flexDirection: "row", justifyContent: "space-between" }}>
                        <View style={{ width: "25%", alignContent: "flex-start", alignItems: "flex-start", padding: 2, paddingVertical: 10, }}>
                            <Text style={{ fontFamily: "mons-e", textAlign: "left" }}>Prix en gros</Text>
                            <Text style={{ fontFamily: "mons-b", textAlign: "left", fontSize: Dims.subtitletextsize, color: Colors.primaryColor }}>{`${prix_max} ${devise}` ?? "---"}</Text>
                        </View>
                        <View style={{ width: "25%", alignContent: "center", alignItems: "center", padding: 2, paddingVertical: 10, }}>
                            <Text style={{ fontFamily: "mons-e", textAlign: "center" }}>Prix détail</Text>
                            <Text style={{ fontFamily: "mons-b", textAlign: "center", fontSize: Dims.subtitletextsize, color: Colors.primaryColor }}>{`${prix_min} ${devise}` ?? "---"}</Text>
                        </View>
                        {/* <View style={{ width: "25%", alignContent: "center", alignItems: "center", padding: 2, paddingVertical: 10 }}>
                        ...
                        <View style={{ width: "50%", alignContent: "center", alignItems: "center", backgroundColor: Colors.pillColor, padding: 2, paddingVertical: 10, borderRightColor: Colors.primaryColor, borderRightWidth: 4 }}>
                            ...
                            <Text style={{ fontFamily: "mons-b", textTransform: "uppercase", textAlign: "center", fontSize: Dims.subtitletextsize, color: Colors.primaryColor }}>{cultures ?? "---"}</Text>
                        </View>
                    </View>
                    ...
                    <View style={{ padding: 2 }}>
                        ...
                        <Text style={{ fontFamily: "mons-b", color: Colors.darkColor }}>{commentaire ?? "---"}</Text>
                    </View>
                </View>
            </TouchableHighlight>
        )
