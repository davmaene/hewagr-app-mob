...
export const EditagriculteurScreen = ({ navigation, route }) => {
    ...
    const [nom, setnom] = React.useState(item && item['nom'] ? item['nom'] : "");
    const [postnom, setpostnom] = React.useState(item && item['postnom'] ? item['postnom'] : "");
    const [prenom, setprenom] = React.useState(item && item['prenom'] && item['prenom'] !== "#" ? item['prenom'] : "");
    const [datenaissance, setdatenaissance] = React.useState(item && item['datenaissance'] !== "#" ? new Date() : new Date());
    const [email, setemail] = React.useState(item && item['email'] && item['email'] !== "#" ? item['email'] : "");
    const [phone, setphone] = React.useState(item && item['phone'] ? item['phone'] : "");
    const [adresse, setadresse] = React.useState(item && item['adresse']);
    const [idvillage, setidvillage] = React.useState(item && item['idvillage']);
    const [idvillages, setidvillages] = React.useState([]);
    const [gender, setgender] = React.useState(item && item['genre'] ? item['genre'] : "");
    const [d, setd] = React.useState(datenaissance !== "#" ? new Date(datenaissance).getDay().toString() : "1");
    const [m, setm] = React.useState(datenaissance !== "#" ? new Date(datenaissance).getMonth().toString() : "12");
    const [y, sety] = React.useState(datenaissance !== "#" ? new Date(datenaissance).getFullYear().toString() : "2000");
                ...
                if(gender.length > 0){
                    if(phone.length > 0){
                        // if(email.length > 0){
                            ...
                            onRunExternalRQST({
                                method: "PUT",
                                url: `/agriculteurs/agriculteur/${item && item['id']}`,
                                data: {
                                    nom,
                                    postnom,
                                    prenom,
                               ...
                               if(done && done['status'] === 200){
                                    ...
                                    Toast.show({
                                        type: 'success',
                                        text1: `Mis à jour`,
                                        text2: `Les mis à jour ont réussie avec succès !`,
                                    });
                                //  onRunRawQRY({
                                //     table: `__tbl_user`,
                        ...
                        //         text2: 'Ajouter une adresse email !',
                        //     });
                        // }
                    }else{
                        Toast.show({
                            type: 'error',
                            text1: 'Valeur obligatoire',
                            text2: 'Ajouter un numéro de téléphone !',
                        });
                    }
                }else{
                    Toast.show({
    ...
    const loadCooperatives = async () => {
        ...
        await onRunExternalRQST({
            ...
            if(liste && liste['status'] === 200){
                setcoops(liste && liste['data'] && liste['data']['liste'])
                setisloading(false);
            }else{
                Toast.show({
                    type: 'error',
    ...
    return(
        <>
           <Title navigation={navigation} title={`${item && item['nom']} ${item && item['postnom']}`} subtitle={"Modification des informations"} />
           <ScrollView 
                    ...
                    <View style={{width: "85%", alignSelf: "center", marginTop: Dims.bigradius }}>
                        ...
                        <View style={{width: "100%", height: 65, flexDirection: "column", marginTop: 15}}>
                            ...
                            <View style={ inputGroup.container }>
                                ...
                                <View style={[ inputGroup.iconcontainer, { }]}>
                                    <Entypo name="phone" size={ Dims.iconsize } color={ Colors.primaryColor } />
                                </View>
                            </View>
                        </View>
                        {/* -------------------------- */}
                        ...
                        {/* <View style={{width: "100%", height: 65, flexDirection: "column", marginTop: 15}}>
                            ...
                            <View style={[ inputGroup.container, { borderWidth: 0, height: Dims.inputTextHeight + 2, justifyContent: "center", overflow: "visible", paddingVertical: 0 }]}>
                                ...
                                <View style={[ inputGroup.iconcontainer, { backgroundColor: Colors.primaryColor, borderBottomRightRadius: Dims.borderradius, borderTopRightRadius: Dims.borderradius }]}>
                                    <MaterialIcons name="date-range" size={ Dims.iconsize } color={ Colors.whiteColor } />
                                </View>
                            </View>
                        </View> */}
                        {/* -------------------------- */}
                        <View style={{width: "100%", height: 65, flexDirection: "column", marginTop: 15}}>
                            <Text style={{ fontFamily: "mons-b", paddingLeft: 10, color: Colors.primaryColor }}>cooperative </Text>
                            <View style={ inputGroup.container }>
                                <View style={ inputGroup.inputcontainer }>
                                    <Dropdown
                                        ...
                                        valueField="id"
                                        placeholder={'Séléctionner une cooperative'}
                                        searchPlaceholder="Recherche ..."
                                        onChange={item => {
                                            setidvillage(item.id);
                                        }}
                                    />
                                </View>
                                <View style={[ inputGroup.iconcontainer, { }]}>
                        ...
                        {/* <View style={{width: "100%", height: 65, flexDirection: "column", marginTop: 15}}>
                            ...
                            <View style={ inputGroup.container }>
                                ...
                                <View style={[ inputGroup.iconcontainer, { backgroundColor: Colors.primaryColor }]}>
                                    <FontAwesome name="map-o" size={ Dims.iconsize } color={ Colors.whiteColor } />
                                </View>
                            </View>
                        </View> */}
                        {/* -------------------------- */}
